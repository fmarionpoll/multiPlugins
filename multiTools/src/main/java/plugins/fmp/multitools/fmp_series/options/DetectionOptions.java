package plugins.fmp.multitools.fmp_series.options;
...
/**
 * Configuration for detection parameters. Replaces detection-related fields
 * from BuildSeriesOptions.
 */
public class DetectionOptions {
	...
	public boolean isDetectSelectedROIs() {
		return detectSelectedROIs;
	}

	public boolean isDetectAllSeries() {
		return detectAllSeries;
	}
	...
	private int validateBackgroundThreshold(int backgroundThreshold) {
		if (backgroundThreshold < 0 || backgroundThreshold > 255) {
			throw new IllegalArgumentException(
					"Background threshold must be between 0 and 255, got: " + backgroundThreshold);
		}
		return backgroundThreshold;
	}
	...
	public static DetectionOptions fromLegacyOptions(BuildSeriesOptions legacy) {
		return builder().threshold(legacy.threshold).flyThreshold(legacy.flyThreshold)
				...
				.detectCage(legacy.detectCage).detectTop(legacy.detectTop).detectBottom(legacy.detectBottom)
				.detectFlies(legacy.detectFlies).detectSelectedROIs(legacy.detectSelectedROIs)
				.detectAllSeries(legacy.detectAllSeries).selectedIndexes(legacy.selectedIndexes)
				.nFliesPresent(legacy.nFliesPresent).videoChannel(legacy.videoChannel).spotRadius(legacy.spotRadius)
				.diskRadius(legacy.diskRadius).build();
	}
}