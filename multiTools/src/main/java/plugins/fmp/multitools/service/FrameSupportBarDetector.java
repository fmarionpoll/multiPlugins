...
/**
 * CODEX Diagnostic detector for the dark horizontal support above the fly
 * cages.
 */
public final class FrameSupportBarDetector {
	public static final class Result {
		...
		/**
		 * Deeply validated candidates, useful for diagnosing an incomplete 9-divider
		 * fit.
		 */
	...
	public static int[] internalDividerSearchBounds(List<Double> capillaryX, int imageWidth) {
		if (capillaryX == null || capillaryX.size() != 20)
			return null;
		...
		for (int i = 0; i < sorted.size(); i += 2)
			cage.add((sorted.get(i) + sorted.get(i + 1)) / 2.);
		ArrayList<Double> pitches = new ArrayList<Double>();
		for (int i = 1; i < cage.size(); i++)
			pitches.add(cage.get(i) - cage.get(i - 1));
		double pitch = percentile(pitches, .5);
		if (!(pitch > imageWidth / 40. && pitch < imageWidth / 4.))
			return null;
	...
	/**
	 * Approximate first/last occupied cage centre, cage pitch, and occupied cage
	 * count.
	 */
	public static double[] frameGridGuide(List<Double> capillaryX, int imageWidth) {
		if (capillaryX == null || capillaryX.size() < 4 || capillaryX.size() % 2 != 0)
			return null;
		ArrayList<Double> sorted = new ArrayList<Double>(capillaryX);
		Collections.sort(sorted);
		ArrayList<Double> cage = new ArrayList<Double>();
	...
	/**
	 * True when an apparently regular image fit conflicts with capillary-pair
	 * scale.
	 */
	...
	/**
	 * Detects all eleven frame boundaries when some of the twenty capillaries are
	 * absent.
	 */
	public Result detectUsingFrameGrid(double[] pixels, int width, int height, int yMin, int yMax, double[] guide) {
		Result result = detect(pixels, width, height, yMin, yMax);
		if (guide == null || result.lowerY <= 0)
			return result;
		int[] bars = findElevenFrameBars(pixels, width, height, result.lowerY, guide);
		if (bars == null)
			return result;
		result.expectedDividers = 9;
		result.dividers.clear();
		result.dividerCandidates.clear();
		result.frameLeft = bars[0];
		result.frameRight = bars[10];
		result.frameWidth = result.frameRight - result.frameLeft;
		int tickBottom = Math.min(height - 1, result.lowerY + Math.max(25, height / 18));
		for (int i = 1; i < 10; i++)
			result.dividers.add(new Line2D.Double(bars[i], result.lowerY + 2, bars[i], tickBottom));
		return result;
	}

	private Result detect(double[] pixels, int width, int height, int yMin, int yMax, int xMin, int xMax,
			boolean trustedHorizontalBounds, int expectedDividers) {
		Result result = new Result();
		result.expectedDividers = Math.max(2, expectedDividers);
		if (pixels == null || width < 20 || height < 20 || pixels.length < width * height)
			return result;
		...
		for (int y = ya + 1; y <= yb; y++)
			if (row[y - ya] < row[upperY - ya])
				upperY = y;
		double noise = medianAbs(row);
		double strength = -row[upperY - ya];
		if (!(strength > Math.max(1., 2.5 * noise)))
			return result;
		...
		for (int x = xa; x <= xb; x++)
			upperMask[x] = pixels[(upperY - 1) * width + x] - pixels[(upperY + 1) * width + x] > upperThreshold;
		closeSmallGaps(upperMask, Math.max(12, width / 12));
		int[] span = largestRun(upperMask);
		if (span[1] - span[0] < width / 3)
			return result;
		...
		// The lower edge is retained only over openings: dividers have no local
		...
		if (dominantLowerY > 0)
			for (int x = span[0]; x <= span[1]; x++)
				if (lowerMask[x] && Math.abs(lowerY[x] - dominantLowerY) > 4)
					lowerMask[x] = false;
		removeShortRuns(lowerMask, Math.max(5, width / 80));
		for (int x = span[0]; x <= span[1];) {
			while (x <= span[1] && !lowerMask[x])
				x++;
			int start = x;
			while (x <= span[1] && lowerMask[x])
				x++;
			int end = x - 1;
			if (end >= start)
				result.lower.add(new Line2D.Double(start, median(lowerY, start, end), end, median(lowerY, start, end)));
		}
	...
	private static int[] findDividers(double[] p, int w, int h, int xa, int xb, int lowerY,
			List<Integer> displayedCandidates, boolean roiGuidedBounds, int expectedCount, int detectedFrameLeft,
			int detectedFrameRight) {
		if (lowerY <= 0)
			return null;
		...
		for (int x = xa; x <= xb; x++) {
			...
			for (int y = ya; y <= yb; y++)
				sum += p[y * w + x];
			mean[x] = sum / (yb - ya + 1);
			distribution.add(mean[x]);
		}
		double dark = percentile(distribution, .20), light = percentile(distribution, .65);
		double threshold = dark + .42 * (light - dark);
		boolean[] mask = new boolean[w];
		for (int x = xa; x <= xb; x++)
			mask[x] = mean[x] < threshold;
		...
		for (int x = xa; x <= xb;) {
			...
			if (e - s + 1 >= minimumWidth) {
				...
				if (persistsBelowBar(p, w, h, s, e, yb, lowerY, light - dark))
		...
		if (best == null && !roiGuidedBounds) {
			...
			candidates = frameSpanLocalMinima(mean, detectedFrameLeft, detectedFrameRight, expectedCount, minimumWidth);
			best = fitRegular(candidates, w, expectedCount);
		}
	...
	private static ArrayList<Integer> guidedLocalMinima(double[] mean, int xa, int xb, int count, int averagingWidth) {
		...
		for (int k = 0; k < count; k++) {
			...
			for (int x = from; x <= to; x++) {
				double sum = 0.;
				for (int q = x - halfAverage; q <= x + halfAverage; q++)
					sum += mean[q];
				if (sum < bestMean) {
					bestMean = sum;
					bestX = x;
	...
	private static ArrayList<Integer> frameSpanLocalMinima(double[] mean, int frameLeft, int frameRight, int count,
			int averagingWidth) {
		ArrayList<Integer> result = new ArrayList<Integer>();
	...
	private static int[] fitRegular(List<Integer> candidates, int w, int count) {
		...
		for (int i = 0; i < candidates.size(); i++)
			for (int j = i + count - 1; j < candidates.size(); j++) {
				double pitch = (candidates.get(j) - candidates.get(i)) / (count - 1.);
				if (pitch < w / 30. || pitch > w / 5.)
					continue;
				...
				boolean ok = true;
				for (int k = 0; k < count; k++) {
					double expected = candidates.get(i) + k * pitch;
					int nearest = -1;
					double error = Double.MAX_VALUE;
					for (int q = previous + 1; q < candidates.size(); q++) {
						double d = Math.abs(candidates.get(q) - expected);
						if (d < error) {
							error = d;
							nearest = q;
						}
					}
					if (nearest < 0 || error > pitch * .28) {
						ok = false;
						break;
					}
					chosen[k] = candidates.get(nearest);
					previous = nearest;
					cost += error * error;
				}
				if (ok && cost < bestCost) {
	 ...
	 * cage interiors much farther down; a capillary ends, while a broad border has
	 * no lighter region on both sides.
	 */
	private static boolean persistsBelowBar(double[] p, int w, int h, int runStart, int runEnd, int nearBandEnd,
			int lowerY, double localRange) {
		int deepStart = Math.min(h - 2, nearBandEnd + Math.max(8, h / 100));
		int deepEnd = Math.min(h - 2, lowerY + Math.max(160, h / 3));
		if (deepEnd <= deepStart + 12)
			return false;
		...
		if (center - flankGap - flankWidth < 1 || center + flankGap + flankWidth >= w - 1)
			return false;
		...
		for (int y = deepStart; y <= deepEnd; y++) {
			...
			usable++;
			if (left - core > requiredContrast && right - core > requiredContrast)
				persistent++;
		}
			...
			for (int missingLeft = 0; missingLeft <= missing; missingLeft++) {
				...
				for (int k = 0; k < 11; k++) {
					...
					for (int x = from; x <= to; x++) {
						...
						if (contrast > localBest) {
							localBest = contrast;
							bestX = x;
						}
					}
					if (k > 0 && bestX <= bars[k - 1]) {
						ok = false;
						break;
					}
					bars[k] = bestX;
					...
					if (k > 0 && k < 10)
						...
						outerScore += Math.max(-5., Math.min(20., localBest));
				}
	...
	private static double multiDepthContrast(double[] p, int w, int y1, int y2, int x, int core, int flank) {
	...
	private static double percentile(List<Double> values, double p) {
		...
		return values.get(Math.max(0, Math.min(values.size() - 1, (int) Math.round(p * (values.size() - 1)))));
	}
	...
	private static int dominantLevel(int[] levels, boolean[] valid, int min, int max, int radius) {
		...
		for (int level = min; level <= max; level++) {
			...
			for (int x = 0; x < levels.length; x++)
				if (valid[x] && Math.abs(levels[x] - level) <= radius)
					count++;
