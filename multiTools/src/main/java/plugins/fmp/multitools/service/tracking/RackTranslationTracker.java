...
/** Image-zero anchored, translation-only registration of rack junctions. */
public final class RackTranslationTracker {
    public static final class Estimate {
        ...
        Estimate(double dx, double dy, double confidence, boolean reliable, String reason) {
            ...
            this.reliable = reliable; this.reason = reason;
        }
    ...
    public RackTranslationTracker(double[] reference, int width, int height,
            FrameSupportBarDetector.Result rack) {
        if (reference == null || reference.length != width * height || rack == null
                || !rack.found() || rack.lowerY <= rack.upperY || rack.dividers.size() != 9)
            throw new IllegalArgumentException("Image 0 needs a complete rack detection");
        this.reference = reference.clone(); this.width = width; this.height = height;
        ...
        for (Line2D divider : rack.dividers) {
            ...
            if (x - half - radius >= 0 && x + half + radius < width
                    && y - half - radius >= 0 && y + half + radius < height)
    ...
    public Estimate estimate(double[] current, int w, int h) {
        ...
        if (matches.size() < 6) return Estimate.uncertain("insufficient rack contrast or search limit");
        double dx = median(matches, 0), dy = median(matches, 1);
        int inliers = 0; double score = 0;
        for (double[] m : matches)
            if (Math.hypot(m[0] - dx, m[1] - dy) <= 1.5) { inliers++; score += m[2]; }
    ...
    private static double median(List<double[]> values, int axis) {
        ...
        for (double[] v : values) sorted.add(v[axis]);
    ...
    public static Line2D translate(Line2D reference, double dx, double dy) {
        return new Line2D.Double(reference.getX1()+dx, reference.getY1()+dy,
                reference.getX2()+dx, reference.getY2()+dy);
    }
}
