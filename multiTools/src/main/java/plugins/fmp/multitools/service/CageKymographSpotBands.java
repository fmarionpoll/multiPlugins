...
public final class CageKymographSpotBands {
	...
	public CageKymographSpotBands(Spot spot, int y0, int y1Exclusive) {
		this.spot = spot;
		this.y0 = y0;
	...
	/**
	 * Same ordering and heights as {@link CageSpotKymographBuilder}: spots sorted by name,
	 * each band height = clipped ROI bounding-box height.
	 */
	public static List<CageKymographSpotBands> layout(Cage cage, Spots allSpots, int refSizex, int refSizey) {
		...
		for (Spot s : sorted) {
			...
			int y0 = row;
			row += b.height;
			out.add(new CageKymographSpotBands(s, y0, row));
		}
		return out;
	}
