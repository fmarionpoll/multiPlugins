 ...
 * generation. This class provides static methods to generate pixel coordinates
 * along lines between two points or along polylines.
 * 
 * <p>
 * The algorithm is based on Bresenham's line algorithm which efficiently
 * determines the points of an n-dimensional raster that should be selected to
 * form a close approximation to a straight line between two endpoints.
 * </p>
 * 
 * <p>
 * References:
 * <ul>
 * <li>Michael ABRASH (1992) "The Good, the Bad and the Run-sliced", Dr Dobb's
 * Journal 194: 171-191</li>
 * <li><a href=
 * "https://www.javatpoint.com/computer-graphics-bresenhams-line-algorithm">JavaTpoint
 * Tutorial</a></li>
 * <li><a href=
 * "https://gist.github.com/0x414c/3bbd1122a50e4be229ce">Implementation by
 * Alexey Gorishny</a></li>
 * <li><a href=
 * "https://github.com/ArminJo/STMF3-Discovery-Demos/blob/master/lib/graphics/src/thickLine.cpp">Fat
 * line implementation</a></li>
 * <li><a href="http://members.chello.at/~easyfilter/bresenham.html">Bresenham
 * description</a></li>
 * </ul>
 * 
 * <p>
 ...
 * ArrayList&lt;Point2D&gt; points = Arrays.asList(new Point2D.Double(0, 0), new Point2D.Double(5, 3),
...
//    private static final int Y_INDEX = 1;
	...
	/**
	 * Generates pixel coordinates along a line between two points using Bresenham's
	 * algorithm.
	 * 
	 * <p>
	 * This method implements the classic Bresenham line algorithm which efficiently
	 ...
	 * @return an ArrayList of int arrays, where each array contains [x, y]
	 *         coordinates
	 ...
	 * 
	 * <p>
	 * This method connects consecutive points in the list using straight lines
	 * generated by Bresenham's algorithm. The resulting pixel list represents the
	 * complete path along the polyline.
	 * </p>
	 * 
	 * @param pointsList list of Point2D objects defining the polyline vertices
	 * @return an ArrayList of int arrays, where each array contains [x, y]
	 *         coordinates
	 * @throws IllegalArgumentException if pointsList is null, empty, or has fewer
	 *                                  than 2 points
	 */
	public static ArrayList<int[]> getPixelsAlongLineFromROI2D(ArrayList<Point2D> pointsList) {
		...
		try {
			...
			for (int i = 1; i < pointsList.size(); i++) {
				...
				ArrayList<int[]> segmentPixels = getPixelsBetween2Points((int) currentPoint.getX(),
						(int) currentPoint.getY(), (int) nextPoint.getX(), (int) nextPoint.getY());
	...
	/**
	 * Convenience method that accepts a list of Point2D objects (not necessarily
	 * ArrayList).
	 * 
	 * @param pointsList list of Point2D objects defining the polyline vertices
	 * @return an ArrayList of int arrays, where each array contains [x, y]
	...
	public static ArrayList<int[]> getPixelsAlongPolyline(java.util.List<Point2D> pointsList) {
		...
		ArrayList<Point2D> arrayList = pointsList instanceof ArrayList ? (ArrayList<Point2D>) pointsList
				: new ArrayList<>(pointsList);

		return getPixelsAlongLineFromROI2D(arrayList);
	}

	/**
	 * Gets the total number of pixels that would be generated for a line between
	 * two points. This method is useful for pre-allocating collections or
	 ...
	 */
	private static void generateLineXPrimary(ArrayList<int[]> linePixels, int currentX, int currentY, int targetX,
			int deltaX, int deltaY, int incrementX, int incrementY) {
	 ...
	 */
	private static void generateLineYPrimary(ArrayList<int[]> linePixels, int currentX, int currentY, int targetY,
			int deltaX, int deltaY, int incrementX, int incrementY) {
		int doubleDeltaX = deltaX << DOUBLE_SHIFT;
