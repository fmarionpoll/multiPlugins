 ...
 * available memory to optimize processing performance while preventing memory
 * overflow.
 */
public class AdaptiveBatchSizer {
	...
	public void initialize(int totalFrames, long availableMemoryMB) {
		...
		int optimalBatchSize = (int) Math.min(maxBatchSize, Math.max(minBatchSize, availableMemoryMB / 400)); // 400
