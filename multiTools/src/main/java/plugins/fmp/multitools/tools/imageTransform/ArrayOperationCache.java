...
/**
 * Performance optimization cache for image array operations. Eliminates the
 * massive code duplication in array conversions that exists across 20+
...
public class ArrayOperationCache {
	...
	/**
	 * Gets RGB arrays for an image, with caching for performance. This method
	 * replaces the repeated code pattern found in 20+ transform classes.
	 * 
	...
	/**
	 * Performs optimized linear combination of RGB channels. Replaces the repeated
	 * manual loops found in multiple transform classes.
	 * 
	...
	private String generateCacheKey(IcyBufferedImage image) {
		return String.format("img_%d_%d_%d_%d", image.getWidth(), image.getHeight(), image.getSizeC(),
	...
	/**
	 * Extracts RGB arrays from an image. This centralizes the array extraction
	 * logic used throughout the transform classes.
	 */
