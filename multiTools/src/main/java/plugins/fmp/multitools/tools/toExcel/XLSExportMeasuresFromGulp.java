...
public class XLSExportMeasuresFromGulp extends XLSExport {
	...
	protected int exportResultType(Experiment exp, int col0, String charSeries, EnumResults resultType,
		...
		if (options.exportLayoutMode == ExportLayoutMode.NORMALIZED && supportsNormalized(resultType)) {
			try {
				options.resultType = resultType;
				return exportNormalizedGulpResultType(exp, charSeries, resultType);
	...
	private static boolean supportsNormalized(EnumResults resultType) {
		return resultType != EnumResults.MARKOV_CHAIN && resultType != EnumResults.AUTOCORREL
				&& resultType != EnumResults.CROSSCORREL && resultType != EnumResults.CROSSCORREL_LR;
	}

	private int exportNormalizedGulpResultType(Experiment exp, String charSeries, EnumResults resultType)
		...
		boolean sparse = resultType == EnumResults.NBGULPS || resultType == EnumResults.AMPLITUDEGULPS
