...
public class XLSExportMeasuresFromCapillary extends XLSExport {
	...
	/** One SERIES row per cage; DATA columns sum + pi (no capillary ID trick). */
	private int exportNormalizedCageLr(Experiment exp, String charSeries, EnumResults resultType,
		...
		for (Cage cage : exp.getCages().getCageList()) {
			...
			if (relation == ExportTimePolicy.Relation.COARSER) {
				long step = resultsOptions.buildExcelStepMs;
				double[] sumRegrouped = resultType == EnumResults.SUMGULPS_LR
						? ExportTimePolicy.regroupSum(timesMs, sumValues, step)
						: ExportTimePolicy.regroupHoldLast(timesMs, sumValues, step);
	...
	/**
	 * Writes only the COMMON and CAP descriptors for capillary-based exports.
	 */
	@Override
