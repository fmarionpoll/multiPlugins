...
public class EzVarChart1D extends EzVar<double[]> {
	 ...
	 * 
	 * @param varName           the name of the variable (as it will appear on the
	 *                          interface)
	 * @param defaultValues     the list of possible values the user may choose from
	 * @param defaultValueIndex the index of the default selected value
	 * @param allowUserInput    set to true to allow the user to input its own value
	 *                          manually, false otherwise
	 * @throws NullPointerException if the defaultValues parameter is null
	 */
	private EzVarChart1D(String varName, double[][] defaultValues, int defaultValueIndex, boolean allowUserInput)
			throws NullPointerException {
		super(new VarLineChart(varName, defaultValues == null ? null : defaultValues[defaultValueIndex]), defaultValues,
