...
public class PannableXYPlot extends XYPlot implements Pannable {
	 ...
	 * 
	 * @return <code>true</code> if domain axis is pannable, otherwise
	 *         <code>false</code>
	 */
	 ...
	 * 
	 * @return <code>true</code> if range axis is pannable, otherwise
	 *         <code>false</code>
	 */
	...
	public void panRangeAxis(double panRange, PlotRenderingInfo info, Point2D source) {
		double fullRange = (getOrientation() == PlotOrientation.HORIZONTAL ? info.getDataArea().getWidth()
				: info.getDataArea().getHeight());

		for (int i = 0; i < getRangeAxisCount(); i++) {
			ValueAxis rangeAxis = getRangeAxis(i);

			double rangeMove = (fullRange != 0
					? Math.abs(rangeAxis.getLowerBound() - rangeAxis.getUpperBound()) * panRange / fullRange
					: 0);
	...
	public void panDomainAxis(double panRange, PlotRenderingInfo info, Point2D source) {
		double fullRange = (getOrientation() == PlotOrientation.HORIZONTAL ? info.getDataArea().getHeight()
				: info.getDataArea().getWidth());

		for (int i = 0; i < getDomainAxisCount(); i++) {
			ValueAxis domainAxis = getDomainAxis(i);

			double rangeMove = (fullRange != 0
					? Math.abs(domainAxis.getLowerBound() - domainAxis.getUpperBound()) * panRange / fullRange
					: 0);
