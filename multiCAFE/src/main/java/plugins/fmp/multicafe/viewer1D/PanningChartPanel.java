...
public class PanningChartPanel extends ChartPanel implements MouseWheelListener {
	 ...
	 *                   for the chart.
	 */
	public PanningChartPanel(JFreeChart chart, boolean properties, boolean save, boolean print, boolean zoom,
			boolean tooltips) {
	 ...
	 */
	public PanningChartPanel(JFreeChart chart, int width, int height, int minimumDrawWidth, int minimumDrawHeight,
	...
	private void move(Point2D startPoint, int oldX, int oldY, int newX, int newY) {
		...
		/*
		 * this is selection of move direction. e.g. dragged to right -> what does this
		 * mean ? move selection to right or left ? value of -1 is the typical graphic
		 * program behaviour
		 */
			...
			if (getChart().getPlot() instanceof Pannable) {
				...
				if (panningPlot.isDomainPannable()) {
					if (panningPlot.getOrientation() == PlotOrientation.VERTICAL) {
						panningPlot.panDomainAxis(diffX, getChartRenderingInfo().getPlotInfo(), startPoint);
					} else {
						panningPlot.panDomainAxis(diffY, getChartRenderingInfo().getPlotInfo(), startPoint);
					}
				}

				if (panningPlot.isRangePannable()) {
					if (panningPlot.getOrientation() == PlotOrientation.VERTICAL) {
						panningPlot.panRangeAxis(diffY, getChartRenderingInfo().getPlotInfo(), startPoint);
					} else {
						panningPlot.panRangeAxis(diffX, getChartRenderingInfo().getPlotInfo(), startPoint);
					}
				}
			}
		} finally {
	...
	public void mouseWheelMoved(MouseWheelEvent e) {
		...
		/*
		 * getWheelRotation < 0 -> wheel moved up (away from user) getWheelRotation > 0
		 * -> wheel moved down (towards the user) take wheel moved away from user for
		 * zoom in.
		 */
	...
	private void zoomIn(Point2D point) {
		...
		PlotRenderingInfo plotInfo = (subPlot >= 0 ? getChartRenderingInfo().getPlotInfo().getSubplotInfo(subPlot)
		...
		for (int di = 0; di < getChart().getXYPlot().getDomainAxisCount(); di++) {
			...
			domainAxis.zoomRange(domainRelPos * (1 - getZoomFactor()),
					domainRelPos + getZoomFactor() * (1 - domainRelPos));
		}
		...
		for (int ri = 0; ri < getChart().getXYPlot().getRangeAxisCount(); ri++) {
			...
			rangeAxis.zoomRange(rangeRelPos * (1 - getZoomFactor()), rangeRelPos + getZoomFactor() * (1 - rangeRelPos));
		}
	}
	...
	private void zoomOut(Point2D point) {
		...
		for (int di = 0; di < getChart().getXYPlot().getDomainAxisCount(); di++) {
			...
			domainAxis.zoomRange(-1 * domainRelPos * (1 - getZoomFactor()),
					1.0 + (1 - domainRelPos) * (1 - getZoomFactor()));
		}
		...
		for (int ri = 0; ri < getChart().getXYPlot().getRangeAxisCount(); ri++) {
			...
			rangeAxis.zoomRange(-1 * rangeRelPos * (1 - getZoomFactor()),
					1.0 + (1 - rangeRelPos) * (1 - getZoomFactor()));
		}
