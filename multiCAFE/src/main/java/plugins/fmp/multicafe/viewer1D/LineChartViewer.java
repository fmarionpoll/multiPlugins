...
public class LineChartViewer extends SwingVarEditor<double[]> {
	...
	protected JComponent createEditorComponent() {
		...
		chart = ChartFactory.createXYLineChart(variable.getName(), "", "", xyDataset, PlotOrientation.VERTICAL, false,
				true, true);
		chartPanel = new PanningChartPanel(chart, 512, 300, 300, 150, 10000, 10000, false, false, true, false, true,
				true);
	...
	/**
	 * Indicates whether and how this component should resize horizontally if the
	 * container panel allows resizing. If multiple components in the same panel
	 * support resizing, the amount of extra space available will be shared between
	 * all components depending on the returned weight (from 0 for no resizing to 1
	 * for maximum resizing).<br/>
	 * By default, this value is 1.0 (horizontal resizing is always allowed to fill
	 * up the maximum amount of space)
	 * 
	...
	/**
	 * Indicates whether and how this component should resize vertically if the
	 * container panel allows resizing. If multiple components in the same panel
	 * support resizing, the amount of extra space available will be shared between
	 * all components depending on the returned weight (from 0 for no resizing to 1
	 * for maximum resizing).<br/>
