...
public class FilterPanel extends JPanel {
	...
	/**
	 * Rebuilds the descriptor index from the full series so Select… dialogs show
	 * current descriptor values after Edit.
	 */
		...
		parent0.descriptorIndex.preloadFromCombo(src, new Runnable() {
			...
			public void run() {
				try {
					...
					if (parent0.paneExperiment != null && parent0.paneExperiment.tabInfos != null)
						parent0.paneExperiment.tabInfos.initCombos();
					if (parent0.paneBrowse != null && parent0.paneBrowse.editPanel != null)
						parent0.paneBrowse.editPanel.initEditCombos();
					updateIndexStatus();
				} finally {
					progress.close();
